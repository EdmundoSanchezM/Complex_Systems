...
def main():
    ...
    t0 = time.time()
    for longitud in range(26,longitud_max):
        ...
        atractor_22_nombre = os.path.join(
            carpeta_22, 'atractor_22_size_'+str(longitud)+'.txt')
        atractor_54_nombre = os.path.join(
            carpeta_54, 'atractor_54_size_'+str(longitud)+'.txt')
        
        with open(atractor_22_nombre, 'w') as fp:
            fp.write('\n'.join('{} {}'.format(x[0],x[1]) for x in atractoresList_22))
        with open(atractor_54_nombre, 'w') as fp:
            fp.write('\n'.join('{} {}'.format(x[0],x[1]) for x in atractoresList_54))
    t1 = time.time()
    total = t1-t0
    print(total)
