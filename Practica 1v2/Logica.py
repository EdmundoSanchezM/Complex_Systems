...
# Momento 2 = Sum(x^2*f(x)) - E[x]^2=> Momento 2 = 0*f(x) + 1*f(x) - (1*f(x=1))^2 = f(x=1) - f(x=1)^2
def GraficaVarianza():
    global probabilidadUnoxPaso
    plot4 = plt.figure(3)
    ...
    plt.xlabel("Steps")
    plt.ylabel("Variance")
    plt.plot(probabilidadUnoxPaso -
             np.power(probabilidadUnoxPaso, 2), color="purple")


# E[x] = Sum(x*f(x))=> E[x] = 0*f(x) + 1*f(x) = f(x=1) = Probabilidad de X=1, osea Probabilidad de uno
def GraficaMedia():
    global probabilidadUnoxPaso
    plot3 = plt.figure(2)
...
def GraficaLog10():
    ...
    plt.plot(np.log(unosxPaso), color="blue")
...
# H(x) = - Suma de i = 1 hasta n de p(xi)logbp(xi) donde p(xi) es la probabilidad de un evento y b = 2 {0,1}
def GraficaEntropiaShannon():
    global probabilidadUnoxPaso
    probabilidadCeroxPaso = 1 - probabilidadUnoxPaso
    h_x = np.negative((probabilidadUnoxPaso*np.log2(probabilidadUnoxPaso)) +
                      (probabilidadCeroxPaso*np.log2(probabilidadCeroxPaso)))
    plot1 = plt.figure(5)
    plt.title("Shannon entropy")
    plt.xlabel("Steps")
    plt.ylabel("Value")
    plt.plot(h_x, color="orange")
...
def Animation_Automata(i):
    if(i != 0):
        ...
        automataCelular[i, :] = calculoPaso(
            automataCelular[i-1, :], regla).astype(np.int8)
...
def Animation_Varianza(i):
    global unosxPaso
    global probabilidadUnoxPaso
    valor_y_varianza.append(
        probabilidadUnoxPaso[i]-np.power(probabilidadUnoxPaso[i], 2))
...
def Animation_DensidadColCentral(i, indexColCentral):
    ...
    if (i > 0):
        valor_x_densidad_central.append(i)
        valor_y_densidad_central.append(
            valor_y_densidad_central[i-1] + datosColCentral[i])
    ax5.plot(valor_x_densidad_central, valor_y_densidad_central, color="black")
    return ax5,


def Animation_Entropia(i):
    global probabilidadUnoxPaso
    probabilidadCeroxPaso = 1 - probabilidadUnoxPaso
    h_x = np.negative((probabilidadUnoxPaso[i]*np.log2(probabilidadUnoxPaso[i])) +
                      (probabilidadCeroxPaso[i]*np.log2(probabilidadCeroxPaso[i])))
    valor_y_entropia.append(h_x)
    ax6.plot(valor_x, valor_y_entropia, color="orange")
    return ax6,
...
def GenerarAutomataCelular(arreglo, nPasos, reglaNumero, cmap, isAnimado):
    ...
    # -------
    # Representamos la regla ahora en base 2 como una cadena (8 bits) es un numero
    ...
    if(isAnimado == 1):
        ...
        plt.ylabel("Value")
        #--Animaciones--#
        anim_automata = FuncAnimation(
            fig, Animation_Automata, frames=nPasos, interval=8, repeat=False)
        anim_densidad = FuncAnimation(
            plot1, Animation_Densidad, frames=nPasos, interval=8, repeat=False)
        anim_log10 = FuncAnimation(
            plot2, Animation_Log10, frames=nPasos, interval=8, repeat=False)
        anim_media = FuncAnimation(plot4, Animation_Media, fargs=(
            len(arreglo),), frames=nPasos, interval=8, repeat=False)
        anim_varianza = FuncAnimation(
            plot3, Animation_Varianza, frames=nPasos, interval=8, repeat=False)
        anim_densidad_central = FuncAnimation(plot5, Animation_DensidadColCentral, fargs=(
            indexColCentral,), frames=nPasos, interval=8, repeat=False)
        anim_entropia = FuncAnimation(
            plot6, Animation_Entropia, frames=nPasos, interval=8, repeat=False)
        plt.show()
    else:
        for pasos in range(1, nPasos):
            # Toma las 3 columnas, para llamar la funcion indiceRegla
            automataCelular[pasos, :] = calculoPaso(
                automataCelular[pasos-1, :], regla)
...
def IniciarRandom(Color0, Color1, Pasos, Regla, TamanioAutomata, isAnimado):
    CelulasIniciales = np.random.randint(
        2, size=TamanioAutomata).astype(np.int8)
    cmap = colors.ListedColormap([Color0, Color1])  # 0,1
    GenerarAutomataCelular(CelulasIniciales, Pasos, Regla, cmap, isAnimado)
    if(isAnimado == 0):
        GraficaDensidad()
...
def IniciarProbabilidad(Color0, Color1, Pasos, Regla, TamanioAutomata, LugaresUnos, isAnimado):
    CelulasIniciales = np.random.choice([0, 1], TamanioAutomata, p=[
                                        1-LugaresUnos, LugaresUnos]).astype(np.int8)
    cmap = colors.ListedColormap([Color0, Color1])  # 0,1
    GenerarAutomataCelular(CelulasIniciales, Pasos, Regla, cmap, isAnimado)
    if(isAnimado == 0):
        GraficaDensidad()
